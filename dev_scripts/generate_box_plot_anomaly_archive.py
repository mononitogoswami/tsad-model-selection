...
data_families.extend(list(set(list(ANOMALY_ARCHIVE_ENTITY_TO_DATA_FAMILY.values())))) 
...
for i, data_family in enumerate(data_families):
    ...
    axes[i].set_title(f"{data_family} (n={len(aggregate_stats['Orcale No-MS/Oracle MS'])})", fontsize=10)    
    _ = axes[i].boxplot([aggregate_stats['Orcale No-MS/Oracle MS'],
