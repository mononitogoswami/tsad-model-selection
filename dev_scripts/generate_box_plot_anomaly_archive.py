...
# Results on the Anomaly Archive
import pandas as pd
import os
...
parser = argparse.ArgumentParser(
    description='Create box plots and compute summary statistics.')

parser.add_argument(
    '--metric',
    type=str,
    help='Metric of rank reliability',
    default='influence',
    choices=['influence', 'pagerank', 'averagedistance', 'shapely'])
parser.add_argument(
    '--aggregation_type',
    type=str,
    help='Type of rank aggregation to use when computing influence.',
    default='kemeny',
    choices=['kemeny', 'borda'])
parser.add_argument('--overwrite',
                    type=bool,
                    help='Whether to overwrite results and plots.',
                    default=True)
parser.add_argument('--split',
                    type=str,
                    help='Which data split are the models evaluated on.',
                    choices=['train', 'test'],
                    default='train')
...
if split == 'train':
    SAVE_DIR = '/home/ubuntu/efs/results/'  # NOTE: Pay careful attention to the paths here
elif split == 'test':
    SAVE_DIR = '/home/ubuntu/efs/results_Jul22_f1_test_set/'  # NOTE: Pay careful attention to the paths here
SAVE_DIR_TEST = '/home/ubuntu/efs/results_Jul22_f1_test_set/'

FORECASTING_METRIC_NAMES = ['MAE', 'MSE', 'SMAPE', 'MAPE', 'LIKELIHOOD']
SYNTHETIC_METRIC_NAMES = [
    'wander', 'average', 'spikes', 'contextual', 'flip', 'speedup', 'scale',
    'cutoff'
]
...
evaluated_entities_test = [
    '_'.join(i.split('_')[2:]).split('.')[0] for i in ranking_object_files_test
]

ranking_object_files = np.intersect1d(ranking_object_files,
                                      ranking_object_files_test)
evaluated_entities = np.intersect1d(evaluated_entities_test,
                                    evaluated_entities)
...
PATH_TO_STATS_PKL = os.path.join(
    SAVE_DIR, f'stats_{dataset}_{split}_{metric}_{aggregation_type}.pkl'
) if metric == 'influence' else os.path.join(
    SAVE_DIR, f'stats_{dataset}_{split}_{metric}.pkl')
...
if not overwrite and os.path.exists(PATH_TO_STATS_PKL):
    ...
    for entity in tqdm(evaluated_entities):
        ...
        with open(os.path.join(SAVE_DIR_TEST, dataset, ranking_obj_file),
                  'rb') as f:
        ...
        if ranks.shape[1] != 19:
            print(
                f'Only {ranks.shape[1]} models trained on {entity}! Skipping...',
            )
            continue
        ...
        _, trimmed_kemeny_rank = trimmed_kemeny(
            ranks,
            metric=metric,
        ...
        praucs = ranking_obj_test.models_performance_matrix.iloc[:,
                                                                 0].to_numpy(
                                                                 ).squeeze()
        f1s = ranking_obj_test.models_performance_matrix.iloc[:, 1].to_numpy(
        ).squeeze()
        model_names = np.array(
            list(ranking_obj.models_performance_matrix.index))

        if np.std(praucs) == 0.0:
            continue  # Bad datasets have 0.5 prauc for all models
...
data_families.extend(
    list(set(list(ANOMALY_ARCHIVE_ENTITY_TO_DATA_FAMILY.values()))))
...
for i, data_family in enumerate(data_families):
    ...
    for entity in stats.keys():
        if (data_family != 'All Entities') and (
                ANOMALY_ARCHIVE_ENTITY_TO_DATA_FAMILY[entity] != data_family):
            continue
    ...
    for entity in stats.keys():
        ...
        aggregate_stats['Trimmed Kemeny MS'].append(
            stats[entity]['Trimmed Kemeny Best F-1'][0])
        aggregate_stats['Kemeny MS'].append(
            stats[entity]['Kemeny Best F-1'][0])
        aggregate_stats['Trimmed Borda MS'].append(
            stats[entity]['Trimmed Borda Best F-1'][0])
        ...
        aggregate_stats['Orcale No-MS'].append(
            stats[entity]['Best F-1'][best_model_idx])
    ...
    aggregate_stats['Forecasting-based MS'] = np.median(
        aggregate_stats['Forecasting-based MS'], axis=1)
    aggregate_stats['Centrality-based MS'] = np.median(
        aggregate_stats['Centrality-based MS'], axis=1)
    aggregate_stats['Anomaly Injection-based MS'] = np.median(
        aggregate_stats['Anomaly Injection-based MS'], axis=1)

    # Compute ratios
    aggregate_stats['Orcale No-MS/Oracle MS'] = aggregate_stats[
        'Orcale No-MS'] / (aggregate_stats['Oracle MS'] + 1e-6)
    aggregate_stats['Random MS/Oracle MS'] = aggregate_stats['Random MS'] / (
        aggregate_stats['Oracle MS'] + 1e-6)
    aggregate_stats['Trimmed Kemeny MS/Oracle MS'] = aggregate_stats[
        'Trimmed Kemeny MS'] / (aggregate_stats['Oracle MS'] + 1e-6)
    aggregate_stats['Kemeny MS/Oracle MS'] = aggregate_stats['Kemeny MS'] / (
        aggregate_stats['Oracle MS'] + 1e-6)
    aggregate_stats['Trimmed Borda MS/Oracle MS'] = aggregate_stats[
        'Trimmed Borda MS'] / (aggregate_stats['Oracle MS'] + 1e-6)
    aggregate_stats['Borda MS/Oracle MS'] = aggregate_stats['Borda MS'] / (
        aggregate_stats['Oracle MS'] + 1e-6)
    aggregate_stats['Forecasting-based MS/Oracle MS'] = aggregate_stats[
        'Forecasting-based MS'] / (aggregate_stats['Oracle MS'] + 1e-6)
    aggregate_stats['Centrality-based MS/Oracle MS'] = aggregate_stats[
        'Centrality-based MS'] / (aggregate_stats['Oracle MS'] + 1e-6)
    aggregate_stats['Anomaly Injection-based MS/Oracle MS'] = aggregate_stats[
        'Anomaly Injection-based MS'] / (aggregate_stats['Oracle MS'] + 1e-6)

    aggregate_stats['Orcale No-MS/Oracle MS'][
        aggregate_stats['Orcale No-MS/Oracle MS'] > 1] = 0
    aggregate_stats['Random MS/Oracle MS'][
        aggregate_stats['Random MS/Oracle MS'] > 1] = 0
    aggregate_stats['Trimmed Kemeny MS/Oracle MS'][
        aggregate_stats['Trimmed Kemeny MS/Oracle MS'] > 1] = 0
    aggregate_stats['Kemeny MS/Oracle MS'][
        aggregate_stats['Kemeny MS/Oracle MS'] > 1] = 0
    aggregate_stats['Trimmed Borda MS/Oracle MS'][
        aggregate_stats['Trimmed Borda MS/Oracle MS'] > 1] = 0
    aggregate_stats['Borda MS/Oracle MS'][
        aggregate_stats['Borda MS/Oracle MS'] > 1] = 0
    aggregate_stats['Forecasting-based MS/Oracle MS'][
        aggregate_stats['Forecasting-based MS/Oracle MS'] > 1] = 0
    aggregate_stats['Centrality-based MS/Oracle MS'][
        aggregate_stats['Centrality-based MS/Oracle MS'] > 1] = 0
    aggregate_stats['Anomaly Injection-based MS/Oracle MS'][
        aggregate_stats['Anomaly Injection-based MS/Oracle MS'] > 1] = 0

    axes[i].set_title(
        f"{data_family} (n={len(aggregate_stats['Orcale No-MS/Oracle MS'])})",
        fontsize=10)
    _ = axes[i].boxplot([
        aggregate_stats['Orcale No-MS/Oracle MS'],
        aggregate_stats['Random MS/Oracle MS'],
        ...
        aggregate_stats['Anomaly Injection-based MS/Oracle MS']
                        ...
                        meanline=True,
                        labels=[
                            'Orcale No-MS', 'Random MS', 'Trimmed Kemeny MS',
...
if metric == 'averagedistance':
    ...
    plt.savefig(
        f"box_plot_{dataset}_{split}_{metric}_{aggregation_type}_{datetime.today().strftime(r'%H-%M-%m-%d-%Y')}.pdf",
        bbox_inches='tight')
