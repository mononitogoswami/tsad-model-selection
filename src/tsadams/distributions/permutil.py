import numpy as np
from .mallows_kendall import find_phi, sample, distance, compose
from .mallows_model import phi_to_theta
...
def distance(sigma, tau=None, dist_name='k'):
    ...
    if dist_name == 'k': return distance(sigma, tau)
    # if dist_name == 'h': return distance(sigma, tau)
...
def sample(n, m, model='mm_ken', params=None):
    ...
    # sample = np.zeros((m,n))
    if model == 'mm_ken':
        return sample(m=m, n=n, phi=params)
    elif model == 'pl':
        return plackett_luce_sample(m, n, w=params)
    elif model == 'gmm_ken':
        return sample(m=m, n=n, theta=params)
    return
...
def full_perm_path(n):
    ...
    # [kendall_tau(perm,p[perm]) for p in pu.full_perm_path(n)]
    # ?this is alway
    perm = list(range(n))
    drifts = [perm[:]]
...
#   [(np.argsort(perm), kendall_tau(np.argsort(perm))) for perm in drifts]
