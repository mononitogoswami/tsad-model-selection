...
MODEL_NAMES = [
    'RNN_1', 'RNN_2', 'RNN_3', 'RNN_4', 'LSTMVAE_1', 'LSTMVAE_2', 'LSTMVAE_3',
    'LSTMVAE_4', 'NN_1', 'NN_2', 'NN_3', 'DGHL_1', 'DGHL_2', 'DGHL_3',
    'DGHL_4', 'MD_1', 'RM_1', 'RM_2', 'RM_3'
]
ANOMALY_TYPES = [
    'average', 'contextual', 'cutoff', 'flip', 'noise', 'scale', 'speedup',
    'wander'
]
QUANTITIES = [
    'anomalysizes_type_spikes', 'anomalylabels_type_spikes',
    'entityscores_type_spikes', 'Ta_type_spikes',
    'anomalysizes_type_contextual', 'anomalylabels_type_contextual',
    'entityscores_type_contextual', 'Ta_type_contextual',
    'anomalysizes_type_flip', 'anomalylabels_type_flip',
    'entityscores_type_flip', 'Ta_type_flip', 'anomalysizes_type_speedup',
    'anomalylabels_type_speedup', 'entityscores_type_speedup',
    'Ta_type_speedup', 'anomalysizes_type_noise', 'anomalylabels_type_noise',
    'entityscores_type_noise', 'Ta_type_noise', 'anomalysizes_type_cutoff',
    'anomalylabels_type_cutoff', 'entityscores_type_cutoff', 'Ta_type_cutoff',
    'anomalysizes_type_scale', 'anomalylabels_type_scale',
    'entityscores_type_scale', 'Ta_type_scale', 'anomalysizes_type_wander',
    'anomalylabels_type_wander', 'entityscores_type_wander', 'Ta_type_wander',
    'anomalysizes_type_average', 'anomalylabels_type_average',
    'entityscores_type_average', 'Ta_type_average'
]
...
def pool_random_repetitions(synthetic_predictions):
    ...
    for mn in MODEL_NAMES:
        ...
        for quantity in synthetic_predictions[mn].keys():
            ...
            if f'{measure}_type_{anomaly_type}' not in pooled_synthetic_predictions[
                    mn].keys():
                pooled_synthetic_predictions[mn][
                    f'{measure}_type_{anomaly_type}'] = []
            pooled_synthetic_predictions[mn][
                f'{measure}_type_{anomaly_type}'].append(
                    synthetic_predictions[mn][quantity].reshape((1, -1)))

        for k in pooled_synthetic_predictions[mn].keys():
            pooled_synthetic_predictions[mn][k] = np.concatenate(
                pooled_synthetic_predictions[mn][k], axis=1)
...
def pool_predictions_of_entities(entities,
    ...
    for entity in tqdm(entities):
        ...
        with open(os.path.join(save_dir, dataset, ranking_obj_file),
                  'rb') as f:
        ...
        if len(ranking_obj.synthetic_predictions.keys()) != len(model_names):
            print(
                f'{len(ranking_obj.predictions.keys())} != {len(ranking_obj.synthetic_predictions.keys())}'
            )
            print(f'Fewer than 19 models trained on {entity}! Skipping...')
            continue

        if type == 'synthetic_predictions':  # Pool all the repeititions of the synthetic anomaly injections
            synthetic_predictions = pool_random_repetitions(
                ranking_obj.synthetic_predictions)

        for mn in model_names:  # NOTE: We are not normalizing the anomaly scores of the models
            for q in quantities:
                if type == 'predictions':
                    predictions[mn][q].append(
                        ranking_obj.predictions[mn][q][:, ignore_timesteps:]
...
def evaluate_models_pooled(entities: List[str],
    ...
    predictions = pool_predictions_of_entities(
        entities=entities,
    ...
    synthetic_predictions = pool_predictions_of_entities(
        entities=entities,
        quantities=QUANTITIES,
        save_dir=save_dir,
        dataset=dataset,
    ...
    models_performance_matrix = pd.concat([
        models_prauc_f1, models_forecasting_metrics, models_centrality,
...
def _get_pooled_aggregate_stats_split(select_entities, eval_entities, dataset,
                                      save_dir, n_neighbors,
                                      synthetic_ranking_criterion,
                                      ignore_metric, n_splits,
                                      evaluation_metric, metric, top_k,
                                      top_kr):
    aggregate_stats = {}
    models_performance_matrix_select = evaluate_models_pooled(
        ...
        save_dir=save_dir,
        n_neighbors=n_neighbors,
        synthetic_ranking_criterion=synthetic_ranking_criterion,
        n_splits=n_splits)
    ...
    if evaluation_metric == 'Best F-1':
        ranks = np.concatenate(
            [ranks_by_metrics[:8, :], ranks_by_metrics[8::2, :]],
            axis=0).astype(int)
    elif evaluation_metric == 'PR-AUC':
        ranks = np.concatenate(
            [ranks_by_metrics[:8, :], ranks_by_metrics[9::2, :]],
            axis=0).astype(int)
    ...
    assert len(metric_names) == ranks.shape[
        0], "Number of ranks should be equal to the number of metric names"
...
def get_pooled_aggregate_stats(save_dir: str = r'/home/ubuntu/efs/results/',
    ...
    aggregate_stats_temp = Parallel(n_jobs=n_jobs)(
        ...
        (evaluated_entities_arr[select_index].reshape((
            -1, )), evaluated_entities_arr[eval_index].reshape((-1, )),
         dataset, save_dir, n_neighbors, synthetic_ranking_criterion,
         ignore_metric, n_splits, evaluation_metric, metric, top_k, top_kr)
...
def _get_stats_for_entity(entity, dataset, metric, evaluation_metric, save_dir,
                          verbose, top_k, top_kr):
    if verbose:
        print(42 * "=")
    ...
    praucs = ranking_obj.models_performance_matrix.iloc[:,
                                                        0].to_numpy().squeeze(
                                                        )
    ...
    metric_names = get_metric_names(
        ranking_obj.models_performance_matrix.columns,
        ignore_metric=ignore_metric)
    ...
    stats = {
        'Rank by PR-AUC':
        model_names[rank_prauc],
        'Rank by Best F-1':
        model_names[rank_f1],
        'Trimmed Kemeny Rank':
        model_names[trimmed_kemeny_rank],
        'Trimmed Kemeny PR-AUC':
        praucs[trimmed_kemeny_rank],
        'Trimmed Kemeny Best F-1':
        f1s[trimmed_kemeny_rank],
        'Kemeny Rank':
        model_names[kemeny_rank],
        'Kemeny PR-AUC':
        praucs[kemeny_rank],
        'Kemeny Best F-1':
        f1s[kemeny_rank],
        'Borda Rank':
        model_names[borda_rank],
        'Borda PR-AUC':
        praucs[borda_rank],
        'Borda Best F-1':
        f1s[borda_rank],
        'Trimmed Borda Rank':
        model_names[trimmed_borda_rank],
        'Trimmed Borda PR-AUC':
        praucs[trimmed_borda_rank],
        'Trimmed Borda Best F-1':
        f1s[trimmed_borda_rank],
        'Partial Borda Rank':
        model_names[partial_borda_rank],
        'Partial Borda PR-AUC':
        praucs[partial_borda_rank],
        'Partial Borda Best F-1':
        f1s[partial_borda_rank],
        'Partial Trimmed Borda PR-AUC':
        praucs[partial_trimmed_borda_rank],
        'Partial Trimmed Borda Best F-1':
        f1s[partial_trimmed_borda_rank],
        'Partial Trimmed Partial Borda PR-AUC':
        praucs[partial_trimmed_partial_borda_rank],
        'Partial Trimmed Partial Borda Best F-1':
        f1s[partial_trimmed_partial_borda_rank],
        'Most Reliable Metric PR-AUC':
        praucs[top_reliability_metric_rank],
        'Most Reliable Metric F-1':
        f1s[top_reliability_metric_rank],
        'Most Reliable Metric (Partial) PR-AUC':
        praucs[top_partial_reliability_metric_rank],
        'Most Reliable Metric (Partial) F-1':
        f1s[top_partial_reliability_metric_rank],
        'Max PR-AUC':
        np.max(praucs),
        'Max Best F-1':
        np.max(f1s),
...
def get_stats_dict(dataset: str = 'smd',
    ...
    PATH_TO_STATS_PKL = os.path.join(
        save_dir, f'stats_{dataset}_{metric}_{evaluation_metric}.pkl'
    ) if metric == 'influence' else os.path.join(
        save_dir, f'stats_{dataset}_{metric}_{evaluation_metric}.pkl')
    ...
    stats = Parallel(n_jobs=n_jobs)(delayed(
        _get_stats_for_entity)(entity, dataset, metric, evaluation_metric,
                               save_dir, verbose, top_k, top_kr)
...
def get_aggregate_stats(stats: dict, evaluation_metric='Best F-1') -> dict:
    ...
    e = list(stats.keys())[0]
    mn_contextual = [
        i for i in stats[e].keys()
        if (('contextual' in i) and (evaluation_metric in i))
    ][0].split('(')[1].split(')')[0]
    mn_spikes = [
        i for i in stats[e].keys()
        if (('spikes' in i) and (evaluation_metric in i))
    ][0].split('(')[1].split(')')[0]

    for entity in stats.keys():
        aggregate_stats['Trimmed Kemeny MS'].append(
            stats[entity][f'Trimmed Kemeny {evaluation_metric}'][0])
        aggregate_stats['Kemeny MS'].append(
            stats[entity][f'Kemeny {evaluation_metric}'][0])
        aggregate_stats['Trimmed Borda MS'].append(
            stats[entity][f'Trimmed Borda {evaluation_metric}'][0])
        aggregate_stats['Borda MS'].append(
            stats[entity][f'Borda {evaluation_metric}'][0])
        aggregate_stats['Partial Borda MS'].append(
            stats[entity][f'Partial Borda {evaluation_metric}'][0])
        aggregate_stats['Oracle MS'].append(
            stats[entity][f'Max {evaluation_metric}'])
        aggregate_stats['Random MS'].append(
            np.mean(stats[entity][f'{evaluation_metric}']))
        aggregate_stats['Oracle No-MS'].append(
            stats[entity][f'{evaluation_metric}'][best_model_idx])

        # Model selection based-on single metrics
        aggregate_stats['MSE-based MS'].append(
            stats[entity][f'Predicted {evaluation_metric} (MSE)'][0])
        aggregate_stats['Likelihood-based MS'].append(
            stats[entity][f'Predicted {evaluation_metric} (LIKELIHOOD)'][0])
        aggregate_stats['Centrality-based MS'].append(
            stats[entity][f'Predicted {evaluation_metric} (CENTRALITY_16)'][0])
        aggregate_stats['Contextual Anomaly Injection-based MS'].append(
            stats[entity][f'Predicted {evaluation_metric} ({mn_contextual})']
            [0])
        aggregate_stats['Spikes Anomaly Injection-based MS'].append(
            stats[entity][f'Predicted {evaluation_metric} ({mn_spikes})'][0])
    ...
    for m in keys:
        ...
        aggregate_stats[f'{m}/Oracle MS'] = aggregate_stats[m] / (
            aggregate_stats['Oracle MS'] + 1e-6)
        aggregate_stats[f'{m}/Oracle MS'][
            aggregate_stats[f'{m}/Oracle MS'] > 1] = 0
...
def get_anomaly_scores_labels(save_dir, dataset):
    ...
    for entity in tqdm(evaluated_entities):
        ...
        for mn in model_names:
            anomaly_scores = ranking_obj.predictions[mn][
                'entity_scores'].squeeze()
            anomaly_labels = ranking_obj.predictions[mn][
                'anomaly_labels'].squeeze()
...
def evaluate_model_performance(entities,
                               stats,
                               anomaly_scores_all_entities,
                               anomaly_labels_all_entities,
                               rank_name=None,
    ...
    for entity in entities:
        _best_model = stats[entity][rank_name][
            0] if best_model is None else best_model
    ...
    _, _, F1, PR_AUC, _ = adjusted_precision_recall_f1_auc(
        y_true=concat_labels, y_scores=concat_scores, n_splits=n_splits)
    return F1, PR_AUC


def evaluate_all_model_performance(entities,
                                   anomaly_scores_all_entities,
                                   anomaly_labels_all_entities,
                                   return_value='mean',
                                   n_splits=100):
    ...
    for i in range(concat_scores.shape[0]):
        _, _, F1, PR_AUC, _ = adjusted_precision_recall_f1_auc(
            y_true=concat_labels[i, :],
            y_scores=concat_scores[i, :],
            n_splits=n_splits)
        F1s.append(F1)
...
def get_aggregate_stats_concat(stats,
                               anomaly_scores_all_entities,
                               anomaly_labels_all_entities,
    ...
    RANK_NAMES = [
        'Trimmed Kemeny', 'Kemeny', 'Trimmed Borda', 'Borda', 'Partial Borda'
    ]
    ...
    for eval_index, select_index in tqdm(kf.split(evaluated_entities_arr)):
        ...
        for ranking in RANK_NAMES:
            ...
            F1, PR_AUC = evaluate_model_performance(
                entities=entities,
                rank_name=rank_name,
                stats=stats,
                ...
                n_splits=100)
            if evaluation_metric == 'Best F-1':
                aggregate_stats[f'{ranking} MS'].append(F1)
            elif evaluation_metric == 'PR-AUC':
                aggregate_stats[f'{ranking} MS'].append(PR_AUC)
        ...
        F1, PR_AUC = evaluate_model_performance(
            ...
            n_splits=100)
        if evaluation_metric == 'Best F-1':
            aggregate_stats['MSE-based MS'].append(F1)
        elif evaluation_metric == 'PR-AUC':
            aggregate_stats['MSE-based MS'].append(PR_AUC)

        F1, PR_AUC = evaluate_model_performance(
            entities=entities,
            rank_name='LIKELIHOOD Rank',
            stats=stats,
            anomaly_scores_all_entities=anomaly_scores_all_entities,
            anomaly_labels_all_entities=anomaly_labels_all_entities,
            n_splits=100)
        if evaluation_metric == 'Best F-1':
            aggregate_stats['Likelihood-based MS'].append(F1)
        elif evaluation_metric == 'PR-AUC':
            aggregate_stats['Likelihood-based MS'].append(PR_AUC)

        F1, PR_AUC = evaluate_model_performance(
            entities=entities,
            rank_name='CENTRALITY_16 Rank',
            stats=stats,
            anomaly_scores_all_entities=anomaly_scores_all_entities,
            anomaly_labels_all_entities=anomaly_labels_all_entities,
            n_splits=100)
        if evaluation_metric == 'Best F-1':
            aggregate_stats['Centrality-based MS'].append(F1)
        elif evaluation_metric == 'PR-AUC':
            aggregate_stats['Centrality-based MS'].append(PR_AUC)

        e = list(stats.keys())[0]
        rank_name = [
            i for i in stats[e].keys()
            if (('Rank' in i) and ('contextual' in i))
        ][0]
        F1, PR_AUC = evaluate_model_performance(
            entities=entities,
            rank_name=rank_name,
            ...
            n_splits=100)
        if evaluation_metric == 'Best F-1':
            aggregate_stats['Contextual Anomaly Injection-based MS'].append(F1)
        elif evaluation_metric == 'PR-AUC':
            aggregate_stats['Contextual Anomaly Injection-based MS'].append(
                PR_AUC)

        rank_name = [
            i for i in stats[e].keys() if (('Rank' in i) and ('spikes' in i))
        ][0]
        F1, PR_AUC = evaluate_model_performance(
            entities=entities,
            rank_name=rank_name,
            ...
            n_splits=100)
        if evaluation_metric == 'Best F-1':
            aggregate_stats['Spikes Anomaly Injection-based MS'].append(F1)
        elif evaluation_metric == 'PR-AUC':
            aggregate_stats['Spikes Anomaly Injection-based MS'].append(PR_AUC)

        # Random Model Selection
        entities = evaluated_entities_arr[eval_index].reshape((-1, ))
        F1, _ = evaluate_all_model_performance(
            entities=entities,
            anomaly_scores_all_entities=anomaly_scores_all_entities,
            anomaly_labels_all_entities=anomaly_labels_all_entities,
            return_value='mean',
        ...
        if evaluation_metric == 'Best F-1':
            aggregate_stats['Random MS'].append(F1)
        elif evaluation_metric == 'PR-AUC':
            aggregate_stats['Random MS'].append(PR_AUC)

        # Select best model from Selection Set
        entities = evaluated_entities_arr[select_index].reshape((-1, ))
        best_model_idx, _ = evaluate_all_model_performance(
            entities=entities,
            anomaly_scores_all_entities=anomaly_scores_all_entities,
            anomaly_labels_all_entities=anomaly_labels_all_entities,
            return_value='argmax',
        ...
        model_names = list(
            list(anomaly_scores_all_entities.values())[0].keys())
        ...
        if evaluation_metric == 'Best F-1':
            aggregate_stats['Oracle No-MS'].append(F1)
        elif evaluation_metric == 'PR-AUC':
            aggregate_stats['Oracle No-MS'].append(PR_AUC)
