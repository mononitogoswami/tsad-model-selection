...
def mean_reciprocal_rank(y_true: np.ndarray, 
    ...
    sorted_model_preferences = sorted(zip(np.arange(len(y_score)), y_score), key=lambda x : x[1], reverse=True)
    sorted_model_pred_ranks = [i[0] for i in sorted_model_preferences]
    sorted_model_praucs = sorted(zip(np.arange(len(y_true)), y_true), key=lambda x : x[1], reverse=True)
    ...
    for i in range(top_k):
        for rank, model_id in enumerate(sorted_model_true_ranks):
            if model_id == sorted_model_pred_ranks[i]:
                top_k_reciprocal_rank = top_k_reciprocal_rank + (1/(rank + 1))
    
    return top_k_reciprocal_rank/top_k


METRICS_NAMES = ['PR-AUC of Top-1 Predicted Model', 
                 'PR-AUC of Top-k Predicted Model', 
                 'Average PR-AUC', 
                 'Range of PR-AUC',
                 'PR-AUC of Best Model',
                 'PR-AUC of Top-3 Best Models',
                 "Kendall's Tau Corr."
                 ]
def get_metric_names():
    return METRICS_NAMES

def evaluate_model_selection(prauc:np.ndarray, y_pred:np.ndarray, k:int=5)->dict:
    ...
    """
    chosen_model_prauc = prauc[y_pred[0]] # PR-AUC of the model with the highest preference
    ...
    metrics_values = [chosen_model_prauc, 
                      chosen_top_k_prauc, 
                      mean_prauc, 
                      range_prauc, 
                      highest_prauc,
...
def gaussian_likelihood(Y: np.ndarray, 
    ...
    if np.sum(np.isnan(Y_sigma)) > 0: pred_std = np.std(Y - Y_hat, axis=1, keepdims=True) + tol
    else: pred_std = Y_sigma + tol
...
def mse(Y: np.ndarray, Y_hat: np.ndarray, Y_sigma: np.ndarray, mask: Optional[np.ndarray]=None) -> float:
    r"""
    Parameters
    ----------
    Y: np.ndarray
...
def mae(Y: np.ndarray, Y_hat: np.ndarray, Y_sigma: np.ndarray, mask: Optional[np.ndarray]=None) -> float:
    r"""    
    .. math::
...
def mape(Y: np.ndarray, Y_hat: np.ndarray, Y_sigma: np.ndarray, mask: Optional[np.ndarray]=None, tol:float=1e-6) -> float:
    r"""
    .. math::
...
def smape(Y: np.ndarray, Y_hat: np.ndarray, Y_sigma: np.ndarray, mask: Optional[np.ndarray]=None, tol:float=1e-6) -> float:
    r"""
    .. math::
    ...
    if mask is None: 
        return 2 * np.mean(
            np.abs(Y - Y_hat) / (np.abs(Y) + np.abs(Y_hat))
        )
    else:
        return 2 * (np.sum(
            mask*(np.abs(Y - Y_hat) / (np.abs(Y) + np.abs(Y_hat)))
        ))/np.sum(mask)

def prauc(Y:np.ndarray, Y_scores:np.ndarray, segment_adjust:bool=True, n_splits:int=100)->float:
    r"""
    ...
    if not segment_adjust: 
        ...
        PR_AUC = adjusted_precision_recall_f1_auc(Y, Y_scores, n_splits=n_splits)[3]
