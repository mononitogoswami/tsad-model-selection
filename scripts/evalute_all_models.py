...
ENTITIES = [MACHINES, ANOMALY_ARCHIVE_ENTITIES]#, MSL_CHANNELS, SMAP_CHANNELS]
...
def evaluate_model_wrapper(dataset, entity): 
    ...
    rank_model_params['downsampling'] = 10 if dataset in ['anomaly_archive', 'smd'] else None
    logging_hierarchy = [dataset]

    if not OVERWRITE: 
        if logging_obj.check_file_exists(obj_class=logging_hierarchy, obj_name=f'ranking_obj_{entity}'):
...
for d_i, dataset in enumerate(DATASETS): 
    _ = Parallel(n_jobs=N_JOBS)(delayed(evaluate_model_wrapper)(dataset, entities) for entities in ENTITIES[d_i])
