...
from model_trainer.hyperparameter_grids import * # DGHL_TRAIN_PARAM_GRID, DGHL_PARAM_GRID, MD_TRAIN_PARAM_GRID, MD_PARAM_GRID, RM_PARAM_GRID, RM_TRAIN_PARAM_GRID, NN_PARAM_GRID, NN_TRAIN_PARAM_GRID, LSTMVAE_TRAIN_PARAM_GRID, LSTMVAE_PARAM_GRID, RNN_TRAIN_PARAM_GRID, RNN_PARAM_GRID
...
from src.pymad.models.NearestNeighbors import NearestNeighbors
...
class TrainModels(object):
    ...
    def __init__(self, 
        ...
        if training_size > 1.0: raise ValueError('Training size must be <= 1.0')
        ...
        if verbose: 
            ...
            print(f'Using the first {training_size*100}\% of the training data to train the models.')
        
        self.overwrite = overwrite
        
        for e_i in range(self.train_data.n_entities):
            t = self.train_data.entities[e_i].Y.shape[1]
            self.train_data.entities[e_i].Y = self.train_data.entities[e_i].Y[:, :int(training_size*t)]

        # Logger object to save the models
        self.logging_obj = Logger(save_dir=save_dir, overwrite=self.overwrite, verbose=verbose)
        self.logging_hierarchy = [dataset, entity]

        self._VALID_MODEL_ARCHITECTURES = ['DGHL', 'RNN', 'LSTMVAE', 'NN', 'MD', 'RM']
    ...
    def train_dghl(self):
        ...
        train_hyper_param_configurations = list(ParameterGrid(DGHL_TRAIN_PARAM_GRID))
        for train_hyper_params in tqdm(train_hyper_param_configurations):
            for model_hyper_params in tqdm(model_hyper_param_configurations):
                if self.train_data.entities[0].X is not None: # DGHL also considers covariates
                    model_hyper_params['n_features'] = self.train_data.n_features + self.train_data.entities[0].X.shape[0]
                else:
                ...
                if not self.overwrite: 
                    if self.logging_obj.check_file_exists(obj_class=self.logging_hierarchy, obj_name=f"DGHL_{MODEL_ID+1}"):
    ...
    def train_md(self):
        ...
        train_hyper_param_configurations = list(ParameterGrid(MD_TRAIN_PARAM_GRID))
        for train_hyper_params in tqdm(train_hyper_param_configurations):
            for model_hyper_params in tqdm(model_hyper_param_configurations):
                model_hyper_params['n_features'] = self.train_data.n_features
                training_args = TrainingArguments(**train_hyper_params)
                ...
                if not self.overwrite: 
                    if self.logging_obj.check_file_exists(obj_class=self.logging_hierarchy, obj_name=f"MD_{MODEL_ID+1}"):
    ...
    def train_lstmvae(self):
        MODEL_ID = 0
        model_hyper_param_configurations = list(ParameterGrid(LSTMVAE_PARAM_GRID))
        train_hyper_param_configurations = list(ParameterGrid(LSTMVAE_TRAIN_PARAM_GRID))
        for train_hyper_params in tqdm(train_hyper_param_configurations):
            for model_hyper_params in tqdm(model_hyper_param_configurations):
                model_hyper_params['n_features'] = self.train_data.n_features
                training_args = TrainingArguments(**train_hyper_params)
                ...
                if not self.overwrite: 
                    if self.logging_obj.check_file_exists(obj_class=self.logging_hierarchy, obj_name=f"LSTMVAE_{MODEL_ID+1}"):
    ...
    def train_rnn(self):
        ...
        train_hyper_param_configurations = list(ParameterGrid(RNN_TRAIN_PARAM_GRID))
        for train_hyper_params in tqdm(train_hyper_param_configurations):
            for model_hyper_params in tqdm(model_hyper_param_configurations):
                training_args = TrainingArguments(**train_hyper_params)
                ...
                if not self.overwrite: 
                    if self.logging_obj.check_file_exists(obj_class=self.logging_hierarchy, obj_name=f"RNN_{MODEL_ID+1}"):
    ...
    def train_rm(self):
        ...
        train_hyper_param_configurations = list(ParameterGrid(RM_TRAIN_PARAM_GRID))
        for train_hyper_params in tqdm(train_hyper_param_configurations):
            for model_hyper_params in tqdm(model_hyper_param_configurations):
                model = RunningMean(**model_hyper_params)

                if not self.overwrite: 
                    if self.logging_obj.check_file_exists(obj_class=self.logging_hierarchy, obj_name=f"RM_{MODEL_ID+1}"):
    ...
    def train_nn(self, batch_size=32):
        ...
        train_hyper_param_configurations = list(ParameterGrid(NN_TRAIN_PARAM_GRID))
        for train_hyper_params in tqdm(train_hyper_param_configurations):
            for model_hyper_params in tqdm(model_hyper_param_configurations):
                model = NearestNeighbors(**model_hyper_params)

                if not self.overwrite: 
                    if self.logging_obj.check_file_exists(obj_class=self.logging_hierarchy, obj_name=f"NN_{MODEL_ID+1}"):
                        print(f'Model NN_{MODEL_ID+1} already trained!')
                        continue

                dataloader = Loader(dataset=self.train_data, 
