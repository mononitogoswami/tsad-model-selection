...
class InjectAnomalies:
    def __init__(self, 
        ...
        self._VALID_ANOMALY_TYPES = ['spikes', 'contextual', 'flip', 'speedup', 'noise', 'cutoff', 'scale', 'wander', 'average']
    ...
    def compute_window_size(self, T):
        ...
        window_size = min(max(int(np.diff(self.peaks).mean()), self.min_window_size), self.max_window_size)
        if self.verbose:
    ...
    def inject_contextual_anomaly(self, window:np.ndarray, scale:float=0.8):
    ...
    # def inject_contextual_anomaly(self, window:np.ndarray, p:float=0.2, scale:float=0.8):
    #     """To inject an anomaly, compose a linear transformation (AX + b) with the time signal
    #     """
    #     mask = bernoulli.rvs(p=p, size=1)
    #     if mask:
    #         a = norm.rvs(loc=1, scale=scale)
    #         b = norm.rvs(loc=0, scale=scale)
    #         return a*window + b - window
    ...
    def compute_anomaly_properties(self, T):
        ...
        self.anomaly_length = np.random.randint(1, self.max_anomaly_length) # Length of anomaly
        ...
        first_peak_idx = self.peaks[np.random.randint(np.ceil(0.1*n_peaks), n_peaks)] # Such that anomalies don't start at the very beginning
        self.anomaly_start = max(first_peak_idx - self.estimated_window_size//2, 0)
        self.anomaly_end = min(self.anomaly_start + self.anomaly_length*self.estimated_window_size, n_time)

        if self.verbose: 
            print(f'Anomaly start: {self.anomaly_start} end: {self.anomaly_end}')
    ...
    def set_random_anomaly_parameters(self):
        self.amplitude_scaling = np.random.choice([0.25, 0.3, 0.5, 2, 3, 4], size=1).squeeze()
        self.speed = np.random.choice([0.25, 0.3, 0.5, 2, 3, 4], size=1).squeeze()
        self.constant_type = np.random.choice(['quantile', 'noisy_0', 'noisy_1', '0', '1'], size=1).squeeze()
        self.baseline = np.random.choice([-0.3, -0.2, -0.1, 0.1, 0.2, 0.3], size=1).squeeze()
    ...
    def inject_anomalies(self, 
        ...
        if self.verbose: print(f'Feature {self.anomalous_feature} has an anomaly!')
        ...
        if self.verbose: print(f'Correlation scaling vector: {correlation_vec}')
        
        # Compute properties of the injected anomaly
        self.compute_anomaly_properties(T)
        if self.verbose: print(f'Length of anomaly for flip, speedup, noise, cutoff, scale and wander anomalies: {self.anomaly_length}')
        
        if anomaly_type == 'spikes':
            spikes = self.inject_spikes(n=T.shape[1], p=anomaly_propensity, scale=scale)
        ...
        # elif anomaly_type == 'contextual':
            # for t in range(0, T.shape[1], window_size):
                # win = T[:, t:t+window_size]
                # anomaly = self.inject_contextual_anomaly(win, scale=scale, p=anomaly_propensity)
                # timeseries_with_anomalies[:, t:t+window_size] = anomaly*correlation_vec[:, None] + win
        
        elif anomaly_type == 'contextual':    
            anomaly = self.inject_contextual_anomaly(T[:, self.anomaly_start:self.anomaly_end], scale=scale)
            timeseries_with_anomalies[:, self.anomaly_start:self.anomaly_end] = anomaly*correlation_vec[:, None] + T[:, self.anomaly_start:self.anomaly_end]
            ...
            xnew = np.linspace(0, self.anomaly_end-self.anomaly_start-1, int((self.anomaly_end-self.anomaly_start)/self.speed))
            ynew = np.zeros((n_features, len(xnew)))
            
            for i in range(n_features): # Interpolate for each feature
                interpf = interpolate.interp1d(np.arange(self.anomaly_end-self.anomaly_start), 
                                        timeseries_with_anomalies[i, self.anomaly_start:self.anomaly_end])    
                ynew[i, :] = interpf(xnew)

            timeseries_with_anomalies = np.concatenate([
                timeseries_with_anomalies[:, :self.anomaly_start], 
                ynew, 
                timeseries_with_anomalies[:, self.anomaly_end:]], axis=1)
            # Define anomaly scores in an absolute fashion instead
            anomaly_size = np.zeros(timeseries_with_anomalies.shape[1])
            anomaly_size[self.anomaly_start:self.anomaly_start+ynew.shape[1]] = 1
            ...
            if self.constant_type == 'quantile':
                ...
                timeseries_with_anomalies[:, self.anomaly_start:self.anomaly_end] = 0
            elif self.constant_type == '1': 
                timeseries_with_anomalies[:, self.anomaly_start:self.anomaly_end] = 1
