...
class RankModels(object):
    def __init__(self, 
                 ...
                 trained_model_path:str=r'/home/ubuntu/efs/trained_models/', # NOTE: Should be shared between Trainer and RankModels Class
                 downsampling:int=10, # NOTE: Should be shared between Trainer and RankModels Class
                 min_length:int=256, # NOTE: Should be shared between Trainer and RankModels Class
                 root_dir:str='/home/ubuntu/datasets/', # NOTE: Should be shared between Trainer and RankModels Class
                 normalize:bool=True, # NOTE: Should be shared between Trainer and RankModels Class
                 verbose:bool=False 
                 ):
        
        self.verbose=verbose
        self.train_data = load_data(dataset=dataset, group='train', entities=[entity], 
                                    downsampling=downsampling, min_length=min_length, 
                                    root_dir=root_dir, normalize=normalize, verbose=verbose)
        self.test_data = load_data(dataset=dataset, group='test', entities=[entity], 
                                   downsampling=downsampling, min_length=min_length, 
                                   root_dir=root_dir, normalize=normalize, verbose=verbose)
        
        self.TRAINED_MODELS_PATH = os.path.join(trained_model_path, dataset, entity)
        print(f'Path to trained models: {self.TRAINED_MODELS_PATH}')

        self.MODEL_NAMES = [i.split('.')[0] for i in os.listdir(self.TRAINED_MODELS_PATH) if 'pth' in i.split('.')[1]]
        print(f'Models in the model set: {self.MODEL_NAMES}')
        print(f'Total number of models in the model set: {len(self.MODEL_NAMES)}')
    ...
    def get_random_syn_anomaly_params(self):
        ...
        if not self.generated_syn_anomaly_params:
            self.random_states = np.random.randint(0, 10000, size=self.n_repeats)
            self.window_sizes = np.random.choice(a = [32, 64, 128], replace=True, size=self.n_repeats)
            self.generated_syn_anomaly_params = True
        
        if self.verbose:
            print('Synthetic Anomaly Injection Parameters:')
            print(f'Number of repeats: {self.n_repeats} | Random states: {self.random_states} | Window Sizes: {self.window_sizes}')
    ...
    def evaluate_models(self, 
        ...
        for model_name in tqdm(self.MODEL_NAMES):
            with open(os.path.join(self.TRAINED_MODELS_PATH, f'{model_name}.pth'), 'rb') as f: 
                model = t.load(f)
            ...
            self.synthetic_predictions[model_name] = evaluate_model_synthetic_anomalies(
                                                        data=self.test_data if split=='test' else self.train_data,
                                                        model=model, 
                                                        model_name=model_name,
                                                        padding_type='right',
        ...
        self.models_synthetic_anomlies = rank_by_synthetic_anomlies(self.synthetic_predictions, criterion=synthetic_ranking_criterion, n_splits=n_splits)

        self.models_performance_matrix = pd.concat([
            self.models_prauc_f1,
            self.models_forecasting_metrics, 
            self.models_centrality, 
            self.models_synthetic_anomlies], axis=1)
    ...
    def rank_models(self)->Tuple[np.ndarray, np.ndarray]:
        ...
        self.ranks_by_metrics, self.rank_prauc, self.rank_f1 = rank_models(self.models_performance_matrix)
