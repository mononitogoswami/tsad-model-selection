...
#     else:
#         metrics_dict_all = []
#         for model_entity_pair in metrics_dict.keys():
#             mep = model_entity_pair.split('_')
#             if len(mep) == 2: m, e = mep[0], mep[1]
#             elif len(mep) == 3: m, e = '_'.join(mep[:2]), mep[2]
#             else: raise NotImplementedError('Splitting model entity key should only result in 2 or 3 pieces!')
#             if (m in model_name) and (e in entity_name):
