...
ranked_models_scores = sorted(zip(np.arange(n_models), PR_AUCs), key=lambda x: x[1], reverse=True)
...
random_permutations = np.array([np.random.permutation(n_models) for i in range(n_samples)])
...
kendalltau_corr = np.array(Parallel(n_jobs=n_jobs)(delayed(compute_kendalltau)(x=ranked_models_gt, y=random_permutations[i, :]) for i in range(n_samples))) # Parallel

# Choose the top-m positive correlated rankings (m = n_metrics)
sorted_corr = sorted(zip(np.arange(n_samples), kendalltau_corr), key=lambda x: x[1], reverse=True)
...
print(f'Shape of ranking_by_metric: {random_permutations[sample_corr_rankings, :].shape}')

# Declare a logger object to save the data
logging_obj = Logger(save_dir=save_data_path, 
                     overwrite=True, 
                     verbose=True)
# Save the data
logging_obj.save(obj=data_obj, 
                obj_name=f"synthetic_rank_data",
                obj_meta={'n_metrics': n_metrics, 'n_models': n_models, 'n_samples': n_samples, 'random_seed': random_seed}, 
